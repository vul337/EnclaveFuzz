    ...
    with open(tracefile) as trace:
        for line in trace.readlines():
            ...
            if not GenModuleCovInfo:
                ...
                _src = int("0x" + src, 0)
                _src = _src if _src == bad_addr or _src < enclave_base else _src + \
                    getCurrentProgram().getImageBase().getUnsignedOffset()-enclave_base
                _dst = int("0x" + dst, 0)
                _dst = _dst if _dst == bad_addr or _dst < enclave_base else _dst + \
                    getCurrentProgram().getImageBase().getUnsignedOffset()-enclave_base
