...
uptr sgxsan_region_is_poisoned_filtered(uptr beg, uptr size, uint8_t filter, bool ret_addr)
{
    if (!beg)
        return 1;
    if (!size)
    ...
    // then check the SHADOW_GRANULARITY-aligned region by calling
    // mem_is_zero on the corresponding shadow.
    if (!AddressIsPoisonedFiltered(beg, filter) && !AddressIsPoisonedFiltered(end - 1, filter) &&
        (shadow_end <= shadow_beg || mem_is_zero((uint8_t *)shadow_beg,
                                                 shadow_end - shadow_beg, filter)))
    {
        return 0;
    }
    if (ret_addr)
