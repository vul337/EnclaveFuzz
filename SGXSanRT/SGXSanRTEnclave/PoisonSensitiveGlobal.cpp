...
void _PoisonSensitiveGlobal(__slsan_global *globalToBePolluted)
{
    ...
    assert(IsAligned(globalToBePolluted->global_variable_addr, SHADOW_GRANULARITY));
    size_t shadowSpanMinusOne = (globalToBePolluted->size + (SHADOW_GRANULARITY - 1)) / SHADOW_GRANULARITY - 1;
