...
def read_edges(tracefile, base_addr):
    ...
    with open(tracefile) as trace:
        for line in trace.readlines():
            ...
            load_addr = getCurrentProgram().getImageBase().getUnsignedOffset()
            _src = int("0x" + src, 0)
            _src = _src if _src == bad_addr or _src < base_addr else _src + load_addr - base_addr
            _dst = int("0x" + dst, 0)
            _dst = _dst if _dst == bad_addr or _dst < base_addr else _dst + load_addr - base_addr
            addr_src = addr.getAddress(hex(_src).rstrip("L"))
            addr_dst = addr.getAddress(hex(_dst).rstrip("L"))
...
def main():
    ...
    ##
    clear_markup()
    edges = read_edges(args.edge_file, base_addr)
    ...
    while BlockIter.hasNext():
        ...
        _func = getFunctionContaining(addr)
        func = _func.getName() if _func else "Unknown"
    ...
    if args.dump_dir:
        ...
        for blocks, total, func, module in cov_data:
            ...
            enclave_all_bb += total
        if not os.path.exists(args.dump_dir):
            os.makedirs(args.dump_dir)
        with open(os.path.join(args.dump_dir, 'cov_'+time.strftime("%Y_%m_%d_%H_%M_%S", time.localtime(time.time()))), 'w') as cov_file:
            cov_file.write("EnclaveCoverage:\t\t%d/%d=%.2f%%\n" % (enclave_cov_bb, enclave_all_bb, float(enclave_cov_bb) * 100 / enclave_all_bb))
            cov_file.write("InterestingCoverage:\t%d/%d=%.2f%%\n" % (interest_cov_bb, interest_all_bb, float(interest_cov_bb) * 100 / interest_all_bb))
            cov_file.write("Effectiveness:\t\t\t%d/%d=%.2f%%\n" % (interest_cov_bb, enclave_cov_bb, float(interest_cov_bb) * 100 / enclave_cov_bb if enclave_cov_bb else 0))
