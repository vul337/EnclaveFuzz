...
void AddressSanitizer::instrumentAddress(Instruction *OrigIns,
    ...
    if (Exp == 0)
      IRB.CreateCall(AsanMemoryAccessCallback[IsWrite][0][AccessSizeIndex],
                     {AddrLong, IRB.getInt1(hasCmpUser(OrigIns)),
                      globalFuncName, IRB.getInt1(isFuncAtTBridge)});
    else
  ...
  // branch Access_BB, Cmp2_BB
  //
  // Cmp2_BB:
  // IsInEnclave = and ShadowByte, IN_ENCLAVE_FLAG
  // cmp IsInEnclave == 0
  // branch OutEnclaveCheck_BB (Access_BB, IsAtTBridge), SlowPath1_BB
  //
  // SlowPath1_BB:
  // ShadowByte = and ShadowByte, 0x8F
  // cmp ShadowByte == 0
  // branch Access_BB, SlowPath2_BB
  //
  // SlowPath2_BB:
  ...
  BasicBlock *Cmp1_BB = nullptr, *Cmp2_BB = nullptr, *SlowPath1_BB = nullptr,
             *Crash_BB = nullptr, *OutEnclaveCheck_BB = nullptr,
             *Access_BB = nullptr;
  Cmp1_BB = InsertBefore->getParent();
  Access_BB = SplitBlock(Cmp1_BB, InsertBefore);
  Function *curFunc = Cmp1_BB->getParent();
  if (not isFuncAtTBridge) {
    OutEnclaveCheck_BB =
        BasicBlock::Create(*C, "OutEnclaveCheck_BB", curFunc, Access_BB);
  }
  Crash_BB =
      BasicBlock::Create(*C, "Crash_BB", curFunc,
                         OutEnclaveCheck_BB ? OutEnclaveCheck_BB : Access_BB);
  ...
  IRB.CreateCondBr(IsInEnclave, SlowPath1_BB,
                   isFuncAtTBridge ? Access_BB : OutEnclaveCheck_BB,
  ...
  if (not isFuncAtTBridge) {
    IRB.SetInsertPoint(OutEnclaveCheck_BB);
...
void AddressSanitizer::initializeCallbacks(Module &M) {
  ...
  for (int Exp = 0; Exp < 1 /* 2 */; Exp++) {
    for (size_t AccessIsWrite = 0; AccessIsWrite <= 1; AccessIsWrite++) {
      ...
      SmallVector<Type *, 6> Args5 = {IntptrTy, IntptrTy, Type::getInt1Ty(*C),
                                      Type::getInt8PtrTy(*C),
                                      Type::getInt1Ty(*C)};
      SmallVector<Type *, 5> Args4 = {IntptrTy, Type::getInt1Ty(*C),
                                      Type::getInt8PtrTy(*C),
                                      Type::getInt1Ty(*C)};
      ...
      AsanMemoryAccessCallbackSized[AccessIsWrite][Exp] = M.getOrInsertFunction(
          ...
          FunctionType::get(IRB.getVoidTy(), Args5, false));
...
bool AddressSanitizer::instrumentFunction(Function &F,
  ...
  if (!isFuncAtTBridge) {
    for (auto GVStoreInst : GVStoreInsts) {
      if (!suppressInstrumentationSiteForDebug(NumInstrumented))
        FunctionModified = instrumentGlobalPropageteWhitelist(GVStoreInst);
    }
  }
...
bool AddressSanitizer::instrumentGlobalPropageteWhitelist(StoreInst *SI) {
  IRBuilder<> IRB(SI);
...
Value *AddressSanitizer::getEDLPtCount(Instruction *insertPt, Value *obj,
  ...
  size_t _eleSize =
      insertPt->getModule()->getDataLayout().getTypeAllocSize(eleTy);
  assert(_eleSize > 0);
  auto eleSize = ConstantInt::get(IntptrTy, _eleSize);
  ...
  if (edlJson[jsonPtr / "string"] == true or
    ...
    /// EDL: \c [string/wstring] must exist with \c [in]
    assert(edlJson[jsonPtr / "in"] == true);
    ptCnt = IRB.CreateCall(
        edlJson[jsonPtr / "string"] == true ? StrLen : WCSLen, argOp);
  } else if (edlJson[jsonPtr / "c_array_count"].is_number()) {
    /// EDL: C array can't be decorated with \c [count]/[size], and must have
    /// count
  ...
  if (ptCnt) {
    ptCnt = IRB.CreateIntCast(ptCnt, IntptrTy, false);
  }
  return ptCnt;
}

Value *AddressSanitizer::getArg(Value *obj, size_t pos) {
  if (Function *F = dyn_cast<Function>(obj)) {
    return F->getArg(pos);
  } else if (CallInst *CI = dyn_cast<CallInst>(obj)) {
    return CI->getArgOperand(pos);
  } else {
    abort();
  }
}

bool AddressSanitizer::hasPointerElement(Type *type) {
  if (typeHasPointerMap.count(type))
    return typeHasPointerMap[type];
  bool result = _hasPointerElement(type, 0);
  typeHasPointerMap[type] = result;
  return result;
}

bool AddressSanitizer::_hasPointerElement(Type *type, size_t level) {
  bool result = false;
  // start from 1
  level++;
...
#define FOR_LOOP_BEG(insertPt, count)                                          \
  Instruction *forBodyTerm = SplitBlockAndInsertIfThen(                        \
      IRB.CreateICmpSGT(count, ConstantInt::get(IntptrTy, 0), ""), insertPt,   \
      false);                                                                  \
  IRB.SetInsertPoint(forBodyTerm);                                             \
  PHINode *phi = IRB.CreatePHI(IntptrTy, 2, "");                               \
  phi->addIncoming(ConstantInt::get(IntptrTy, 0),                              \
                   forBodyTerm->getParent()->getPrevNode());                   \
  BasicBlock *forBodyEntry = phi->getParent();

#define FOR_LOOP_END(count)                                                    \
  /*! May insert new BB inside, so \c forBodyTerm may not belong to            \
   * \c forBodyEntry BB */                                                     \
  IRB.SetInsertPoint(forBodyTerm);                                             \
  Value *inc =                                                                 \
      IRB.CreateAdd(phi, ConstantInt::get(IntptrTy, 1), "", true, true);       \
...
bool AddressSanitizer::instrumentPtrBridgeCheck(Value *operand,
                                                Instruction *insertPt,
                                                int depth, Value *eleCnt,
                                                Value *operandAddr,
                                                bool checkCurLvPtr) {
  if (depth++ > 10)
    return false;
  ...
  if (PointerType *pointerType = dyn_cast<PointerType>(operandType)) {
    // ignore unsized or function type
    if (!pointerType->getElementType()->isSized() or
        pointerType->getElementType()->isFunctionTy())
      return false;
    size_t _eleSize = insertPt->getModule()->getDataLayout().getTypeAllocSize(
    ...
    Value *eleCntSInt32 =
        eleCnt ? IRB.CreateIntCast(eleCnt, Type::getInt32Ty(*C), true)
               : IRB.getInt32(-1);
    CallInst *isReadable = IRB.CreateCall(
        is_region_readable, {operandInt8ptr, eleSize, eleCntSInt32});
    Instruction *PointerCheckTerm =
        SplitBlockAndInsertIfThen(isReadable, insertPt, false);
    IRB.SetInsertPoint(PointerCheckTerm);

    // now pointer is loadable
    if (checkCurLvPtr)
      IRB.CreateCall(SGXSanBridgeCheck,
                     {operandInt8ptr, eleSize, eleCntSInt32});

    if (eleCnt == ConstantInt::get(IntptrTy, 0)) {
      abort();
    } else if (eleCnt && eleCnt != ConstantInt::get(IntptrTy, 1)) {
      // multi element
      FOR_LOOP_BEG(PointerCheckTerm, eleCnt)
      Value *eleAddr =
          IRB.CreateGEP(pointerType->getElementType(), operand, phi);
      auto ele = IRB.CreateLoad(pointerType->getElementType(), eleAddr);
      instrumentPtrBridgeCheck(ele, PointerCheckTerm, depth, nullptr, eleAddr);
      FOR_LOOP_END(eleCnt)
    } else {
      // one element or don't know count
      auto ele = IRB.CreateLoad(pointerType->getElementType(), operand);
      bool result = instrumentPtrBridgeCheck(ele, PointerCheckTerm, depth,
                                             nullptr, operand);
      if (not result)
        ele->eraseFromParent();
    }
    ...
    for (size_t index = 0; index < structType->getNumElements(); index++) {
      IRB.SetInsertPoint(insertPt);
      Value *element = IRB.CreateExtractValue(operand, index);
      bool result = instrumentPtrBridgeCheck(element, insertPt, depth);
      if (result)
        has_modified = true;
      else {
    ...
    if (!hasPointerElement(arrayType))
      return false;
    bool has_modified = false;
    if (operandAddr) {
      // We have array pointer, thus we can use GEP to get array element pointer
      auto cnt = ConstantInt::get(IntptrTy, arrayType->getNumElements());
      FOR_LOOP_BEG(insertPt, cnt)
      Value *eleAddr = IRB.CreateGEP(arrayType, operandAddr,
                                     {ConstantInt::get(IntptrTy, 0), phi});
      auto ele = IRB.CreateLoad(arrayType->getElementType(), eleAddr);
      instrumentPtrBridgeCheck(ele, insertPt, depth, nullptr, eleAddr);
      FOR_LOOP_END(cnt)
      has_modified = true;
    } else {
...
bool AddressSanitizer::instrumentRealECall(CallInst *RealECall) {
  ...
  /// Instrument \c SGXSanBridgeCheck for each actual parameter of RealEcall
  /// before call it
  for (size_t i = 0; i < RealECall->getNumArgOperands(); i++) {
    Value *arg = RealECall->getArgOperand(i);
    if (isa<PointerType>(arg->getType())) {
      /// EDL: Argument may be a multi-element array pointer. Subfield need to
      /// be filled into whitelist as well
      ...
      instrumentPtrBridgeCheck(arg, RealECall, 0);
    }
  }
  /// Instrument \c WhitelistActive before RealEcall, \c WhitelistDeactive after
  /// RealEcall
...
void AddressSanitizer::declareAdditionalSymbol(Module &M) {
  ...
  // Bridge check
  SGXSanBridgeCheck = M.getOrInsertFunction("SGXSanBridgeCheck",
                                            IRB.getVoidTy(), IRB.getInt8PtrTy(),
                                            IRB.getInt64Ty(), IRB.getInt32Ty());
  is_region_readable =
      M.getOrInsertFunction("is_region_readable", IRB.getInt1Ty(),
  ...
  StrLen = M.getOrInsertFunction("strlen", Type::getInt64Ty(*C),
                                 Type::getInt8PtrTy(*C));
  WCSLen = M.getOrInsertFunction("wcslen", Type::getInt64Ty(*C),
  ...
  if (ClEdlJsonFile.getNumOccurrences() > 0) {
    /// Initialize \c edlJson from xxx.edl.json
    std::ifstream ifs(ClEdlJsonFile);
    if (not ifs.is_open())
      abort();
    std::stringstream buffer;
    buffer << ifs.rdbuf();
    edlJson = ordered_json::parse(buffer.str());
    ifs.close();
    hasEdlJson = true;
  }
}
